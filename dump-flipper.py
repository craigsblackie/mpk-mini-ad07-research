...
PROBE_INTERFACE = "cmsis-dap.cfg"
PROBE_SERIAL = "DAP_Himimoni"
ADAPTER_KHZ = 50
...
def print_instructions():
    print("AD07 instructions (read FULL_DUMP_PICO.md before wiring):")
    print("")
    print("1. Flash the attack firmware to the Pi Pico")
    print("")
    print("2. Connect the Pico through the reviewed AD07 adapter:")
    print("     GND     -> GND      ")
    print("     GPIO 0  -> NO LONGER USED (previously USARTx_RX)")
    print("     GPIO 1  -> USARTx_TX")
    print("     GPIO 2  -> external active-high power-switch EN")
    print("     GPIO 4  -> NRST     ")
    print("     GPIO 5  -> BOOT0    ")
    print("     PB2/BOOT1 -> switched 3.3V through 10K")
    print("     NEVER connect GPIO2 directly to the populated AD07 VDD rail")
    print("")
    print("Where:")
    print("     USART1_TX = PA9")
    print("     USART2_TX = PA2")
...
def openocd_run(interface: str, traget: str, cmds: list):
    # AD07 setup: Flipper Zero DAP Link (CMSIS-DAP v2), deliberately slow SWD.
    # WORKAREASIZE=0 prevents OpenOCD from reserving/altering target SRAM behind
    # the exploit loader's back.  The caller's load_image command writes only
    # the audited payload to SRAM at 0x20000000; no flash command is used.
    splist = [
        "openocd",
        "-f", "interface/" + PROBE_INTERFACE,
        "-c", "adapter serial " + PROBE_SERIAL,
        "-c", "transport select swd",
        "-c", "adapter speed " + str(ADAPTER_KHZ),
